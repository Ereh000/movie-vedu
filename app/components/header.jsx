...
export default function Header() {
  return (
    <header className="bg-transparent text-white shadow-md fixed top-0 left-0 right-0 z-10 ">
      <div className="container mx-auto px-16 py-3 flex justify-between items-center">
        {/* Logo */}
        <div className="flex items-center">
          <Link to="/" className="text-2xl font-bold text-red-500">NETMIRROR</Link>
        </div>
        
        {/* Navigation */}
        <nav className="hidden md:flex space-x-8">
          <Link to="/" className="hover:text-red-400 transition-colors">Home</Link>
          <Link to="/tv-series" className="hover:text-red-400 transition-colors">TV Series</Link>
          <Link to="/movies" className="hover:text-red-400 transition-colors">Movies</Link>
          <Link to="/my-list" className="hover:text-red-400 transition-colors">My List</Link>
        </nav>
